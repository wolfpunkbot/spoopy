...
TO DO:
-- CONVERT MAP FILES TO JSON  -> (with 'var templateBuilder = true') CREATE TEMPLATE BUILDER WITH BLOCK SELECTION AND EXPORT ABILITY SO I CAN MAKE NEW MAPS MUCH MORE QUICKLY
-- ADD SPLASH SCREEN
...
<html lang="en">
	...
	<style>
		*, h1 {
		...
		#gameBoard {
			...
			//outline: 2px solid #69746b;
		}
		.gameLine div {
			background-color: #69746b;
			outline: 2px solid #606A62;
			min-width: 60px;
			min-height: 60px;
			height: 60px;
			width: 60px;
	...
	<body>
		...
		<section>
			<div class=""style=""><!-- container--900 padding-top: 100px; -->
				<div id="myInventory" class="flex">
						<h1>Items Collected:</h1>&nbsp;<div></div>
					</div>
				<div class="container--1200" style="overflow: visible;">
					<div id="gameBoard" class="centered">test</div>
					<div id="joyStick" class="hidden">test</div>
				</div>
		...
		<script>
			...
			var totalMaps = 2;
			var amount = 0;
			var boardBoundaryBlocks, interactable, beamBlocks, levelBones, levelLava, levelIce, levelPushBlocks, levelSpotlights = [];
			var boardSize = 15;
			...
			var level, levelKeys, flip, direction, num, totalItems, startingSquare, helpMsg, endPortal, helpBlockLoc, masterDoorNum, playerTexture, globalMarginTop, globalMarginLeft = '';
			
			//DICTATES THE GLOBAL POSITION OF THE #GAMEBOARD IN ORDER TO KEEP THE PLAYER COMPLETELY CENTERED IN THE SCREEN
			const gameBoardSpacer = 150;
			$('#gameBoard').css('transform','scale(1.25)');
			...
			if (devDebug == true) {
				$('body').append('<div class="centered crossHairBlock">X</div>');
			}
				...
				if (newPos > 0 && newPos <= totalSquares) { //STOP PLAYER FROM MOVING OUTSIDE THE BOARD
					...
					if (canMove == true) {
						...
						if ( $('[data-pushBlockNum="' + newPos + '"]').attr('data-pushBlockNum') != null && $('[data-pushBlockNum="' + newPos + '"]').attr('data-inWater') == 'false' ) {
							var num = 0;
							if (arrow == 'UP') { num = newPos-15; }
							if (arrow == 'DOWN') { num = newPos+15; }
						...
						if (arrow == "LEFT" && canMove == true) {
							...
							//CAMERA CONTROLS
							//KEEP PLAYER CENTERED AND MOVE THE BOARD AROUND
							var numOne = Number($('#gameBoard').css('margin-left').split('px')[0]);
							var numTwo = numOne + gameBoardSpacer;
							$('#gameBoard').css('margin-left', numTwo);
						...
						if (arrow == "UP" && canMove == true) {
							...
							//CAMERA CONTROLS
							//KEEP PLAYER CENTERED AND MOVE THE BOARD AROUND
							var numOne = Number($('#gameBoard').css('margin-top').split('px')[0]);
							var numTwo = numOne + (gameBoardSpacer/2);
							$('#gameBoard').css('margin-top', numTwo);
			...
			function playerInLava() {
				...
				setTimeout(() => {
					...
					$('#gameBoard').css('margin-top', globalMarginTop).css('margin-left', globalMarginLeft);
					fellInLava = false;
					canMove = true;
				}, 1500);
			}
					...
					success: function() {
						...
						$.getScript('assets/levels/map_' + level + '.js').done(function(script, textStatus) {
							if (textStatus == 'success') {
								...
								/*
								var playerX = $('#player')[0].x - 800;
								var playerY = $('#player')[0].x - 800;
